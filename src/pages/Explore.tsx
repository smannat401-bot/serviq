        ...
        {!user ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              ...
              {filteredWorkers
                .filter(pro => user?._id !== pro._id)
                .map((pro) => (
                <motion.div 
                  key={pro._id}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="glass-card overflow-hidden flex flex-col group relative hover:border-brand-electricBlue/30 transition-all cursor-pointer"
                    ...
                    {(!user?._id) ? (
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <button 
                          onClick={(e) => {
                            e.stopPropagation();
                            setSelectedProfile(pro);
                          }}
                          className="w-full py-2.5 glass text-brand-black dark:text-white font-bold rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 transition-colors text-sm"
                        >
                          View Profile
                        </button>
                      </div>
                    ) : user?._id === pro._id ? (
                      <div className="w-full py-3 bg-gray-100 dark:bg-white/5 text-gray-500 font-bold rounded-xl text-center cursor-not-allowed">
                        Your Profile
                      </div>
                    ) : null}
                  </div>
