...
import { 
  ClipboardList, Heart, MessageSquare, Settings, Bell, MapPin, Search, BadgeCheck, Tag, LogOut, Star, Upload, Trash, Phone
} from 'lucide-react';
...
export default function ClientDashboard() {
  ...
  const fetchBookings = () => {
    if (user._id) {
      fetch(`${API_URL}/api/bookings/client/${user._id}`)
        .then(res => res.json())
        .then(data => {
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <main className="flex-1 space-y-8">
            ...
            {activeTab === 'bookings' && (
              ...
              >
                <h2 className="text-xl font-bold text-brand-black dark:text-white mb-6">Past Repairs</h2>
                  ...
                  {bookings.map((booking) => (
                    <div key={booking._id} className="flex justify-between items-center p-5 rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-[#0f172a] hover:shadow-sm transition-all group">
                      <div className="flex gap-4 items-center">
                        ...
                        <div>
                          <h4 className="font-bold text-brand-black dark:text-white">{booking.serviceDetails?.title || 'Service'}</h4>
                      ...
                      <div className="text-right flex flex-col items-end gap-2">
                        <p className="font-bold text-brand-black dark:text-white mb-1">${booking.serviceDetails?.price || booking.basePrice || '0'}</p>
