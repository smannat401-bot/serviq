...
router.post('/login', async (req, res) => {
  try {
    ...
    // Verify role (optional: depending if client/worker login are separate)
    if (user.role !== role) {
    ...
    if (user.isBlocked) {
      return res.status(403).json({ 
        message: 'Your account has been blocked due to repeated cancellations. Please contact support.',
        isBlocked: true
      });
    }
...
// Add Item to Catalog
router.post('/catalog/:id', async (req, res) => {
  try {
...
router.delete('/catalog/:id/:itemId', async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
...
router.patch('/pricing/:id', async (req, res) => {
  try {
    const { baseCharge, distanceRate, travelFee } = req.body;
